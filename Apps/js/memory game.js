...
if (window.localStorage.getItem("allPlayers")) {
  ...
  span.appendChild(document.createTextNode("فارغ"));
...
document.querySelector(".control-btn span:first-child").onclick = function () {
  sweatPrompt("ما هو اسمك ؟", document.querySelector(".name1 span"));
...
function endGame(time = false) {
  ...
  if (boxes.length === blocks.length) {
    ...
    div.appendChild(document.createTextNode(`مبروك، نتيجتك هي ${par}`));
  ...
  if (time === true && boxes.length !== blocks.length) {
    ...
    let div = document.createElement("div");
    div.appendChild(document.createTextNode(`انتهت اللعبة، نتيجتك هي ${par}`));
    div.classList.add("finalMass");
...
function toggleAmPm(timeHour) {
  if (timeHour.endsWith("ص")) {
    // AM = ص
    timeHour = Array.from(timeHour);
    timeHour.splice(timeHour.length - 1, 1, "م");
    return timeHour.join("");
  } else if (timeHour.endsWith("م")) {
    timeHour = Array.from(timeHour);
    timeHour.splice(timeHour.length - 1, 1, "ص");
    return timeHour.join("");
  }
}
function getDateOfGame() {
  ...
  let theDate = `${new Date().getFullYear()}/${
    new Date().getMonth() + 1
  }/${new Date().getDate()} ${timeGame}:${new Date().getMinutes()}ص`;
  if (timeGame > 12) {
    timeGame -= 12;
    theDate = toggleAmPm(
...
function sweatPrompt(txtMaessage, valueReturnVar) {
  ...
  btnOk.innerHTML = "حسنا";
  btnCansel.innerHTML = "اغلق";
